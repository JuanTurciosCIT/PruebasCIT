...
export const TheTeam = ({
  ...
  return (
    <section className={chosenTheme.section}>
      <div>
        <h2 className={`${utils.headingMedium} ${chosenTheme.title}`}>
          {/* {t("team.title")} */}
		  { employeeContent.tittleEn || employeeContent.tittleEs }
        </h2>
        {!router.pathname.includes("about-us") && (
          <p className={`${utils.textMedium} ${chosenTheme.desc}`}>
			{employeeContent?.captionEn || employeeContent?.captionEs}
		  </p>
        )}
        <div>
          {isMobile ? (
                ...
                <SwiperSlide className={chosenTheme.slider} key={employee.id}>
                  <div className={chosenTheme.partnerCard}>
                    ...
                    <div className={chosenTheme.textWrapper}>
                      <h3 className={chosenTheme.name}>{employee.fullName}</h3>
                      <p className={chosenTheme.role}>
